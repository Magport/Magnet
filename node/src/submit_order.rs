...
pub async fn build_rpc_for_submit_order(
	...
	if hash == H256::from([0; 32]) {
		client
			.tx()
			.sign_and_submit_default(&place_order, &signer_keystore)
			.await
			.map_err(|_e| SubmitOrderError::RPCCallException)?;
	} else {
		let latest_block =
			client.blocks().at(hash).await.map_err(|_e| SubmitOrderError::GetBlockError)?;

		let tx_params = Params::new().mortal(latest_block.header(), slot_block.into()).build();

		client
			.tx()
			.sign_and_submit(&place_order, &signer_keystore, tx_params)
			.await
			.map_err(|_e| SubmitOrderError::RPCCallException)?;
	}

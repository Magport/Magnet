...
use asset_transfer::{Error as AssetTransferError, TransferType, XcmAssetTransfers};
use controller::{
...
use xcm_executor::{
	traits::{
		...
		OnResponse, Properties, QueryHandler, QueryResponseStatus, TransactAsset,
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type XcmExecutorConfig: xcm_executor::Config;
	...
	//<<T as pallet::Config>
	impl<T: Config> XcmAssetTransfers for Pallet<T> {
		type IsReserve = <T::XcmExecutorConfig as xcm_executor::Config>::IsReserve;
		type IsTeleporter = <T::XcmExecutorConfig as xcm_executor::Config>::IsTeleporter;
		type AssetTransactor = <T::XcmExecutorConfig as xcm_executor::Config>::AssetTransactor;
	}
...
impl<T: Config> Pallet<T> {
	...
	fn validate_assets_and_find_reserve(
		...
		for asset in assets.iter() {
			...
			let transfer_type = <Self as XcmAssetTransfers>::determine_for(&asset, dest)
	...
	fn do_reserve_transfer_assets(
		...
		let fees_transfer_type =
			<Self as XcmAssetTransfers>::determine_for(&fees, &dest).map_err(Error::<T>::from)?;
	...
	fn teleport_fees_instructions(
		...
		// a trusted chain.
		<Self as XcmAssetTransfers>::AssetTransactor::can_check_out(&dest, &fees, &dummy_context)
			.map_err(|_| Error::<T>::CannotCheckOutTeleport)?;
		<Self as XcmAssetTransfers>::AssetTransactor::check_out(&dest, &fees, &dummy_context);

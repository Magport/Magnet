...
fn to_substrate_account_check() {
	ExtBuilder::default().build().execute_with(|| {
		for pair in DATASET.iter() {
			let pk_expected = Public::from_ss58check_with_version(pair.0).unwrap().0.into();
...
fn account_to_bytes_check() {
	for pair in DATASET.iter() {
		let (pk, _) = Public::from_ss58check_with_version(pair.0).unwrap();
		let bytes = MoveModule::to_move_address(&pk.into()).unwrap().into_bytes();
		assert_eq!(pair.1, hex::encode(bytes));

		let bytes_expected =
			AccountAddress::from_hex_literal(&format!("0x{}", pair.1)).unwrap().to_vec();
...
fn check_protected_address_errors() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let pk_expected =
			Public::from_ss58check_with_version(utils::PROHIBITED_ADDR).unwrap().0.into();

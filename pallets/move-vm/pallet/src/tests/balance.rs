...
fn verify_simple_transfer() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let transaction_bc =
				script_transaction!(script, no_type_args!(), &alice_addr_mv, &bob_addr_mv, &AMOUNT);
...
fn verify_insufficient_balance() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let transaction_bc =
			script_transaction!(script, no_type_args!(), &bob_addr_mv, &alice_addr_mv, &AMOUNT);
...
fn verify_move_script_fails_after_successful_transfer() {
	...
	ExtBuilder::default()
		.with_balances(vec![(alice_addr_32.clone(), BALANCE), (bob_addr_32.clone(), BALANCE)])
		.build()
		.execute_with(|| {

...
fn pseudo_benchmark_gas_costs() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let gas1: u64 = MoveModule::rpc_estimate_gas_execute_script(script_bc).unwrap().gas_used;
		...
		let script_bc = script_transaction!(script, no_type_args!(), &alice_addr_move);
		let gas2: u64 = MoveModule::rpc_estimate_gas_execute_script(script_bc).unwrap().gas_used;
		...
		let gas3: u64 = MoveModule::rpc_estimate_gas_execute_script(script_bc).unwrap().gas_used;
		...
		let gas4: u64 = MoveModule::rpc_estimate_gas_execute_script(script_bc).unwrap().gas_used;
		...
		let script_bc = script_transaction!(script, no_type_args!());
		let gas5: u64 = MoveModule::rpc_estimate_gas_execute_script(script_bc).unwrap().gas_used;
		...
		let script_bc = script_transaction!(script, no_type_args!());
		let gas6: u64 = MoveModule::rpc_estimate_gas_execute_script(script_bc).unwrap().gas_used;

...
fn general_script_no_params_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			last_event(),
			RuntimeEvent::MoveModule(Event::<Test>::ExecuteCalled { who: vec![bob_addr_32] })
		);
	})
}
...
fn eve_cant_execute_multisig_script_without_other_signers_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(execute_script(&eve_addr_32, script.clone(), params.clone(), type_args.clone()));
		assert_eq!(
			last_event(),
		...
		let result =
			execute_script(&eve_addr_32, script.clone(), params.clone(), type_args.clone());
...
fn signer_before_all_possible_vectors_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let params: Vec<&[u8]> =
			vec![&bob, &v_u8, &v_u16, &v_u32, &v_u64, &v_u128, &v_u256, &v_addr, &v_bool];
...
fn signer_after_all_possible_vectors_fails() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let bob = pg.address(&bob_addr_mv);
		let params: Vec<&[u8]> =
			vec![&v_u8, &v_u16, &v_u32, &v_u64, &v_u128, &v_u256, &v_addr, &v_bool, &bob];
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				last_event(),
				RuntimeEvent::MoveModule(Event::<Test>::ExecuteCalled {
					who: vec![dave_addr_32.clone(), alice_addr_32.clone(), eve_addr_32.clone()]
				})
			);

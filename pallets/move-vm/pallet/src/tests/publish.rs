...
fn raw_publish_module_dry_run() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(invalid_publish.is_err(), "managed to publish a module with insufficient gas");
...
fn raw_publish_bundle_dry_run() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(invalid_publish.is_err(), "managed to publish a bundle with insufficient gas");
...
fn publish_module_will_fail_in_protected_memory() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(result.is_err(), "managed to publish a module in the protected memory area");
	})
}
...
fn publish_bundle_will_fail_in_protected_memory() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(result.is_err(), "managed to publish a bundle in the protected memory area");
	})
}

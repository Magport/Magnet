...
contract AssetsBridgeErc20 is Context, IERC20, IERC20Metadata, Pausable, Ownable, IAssetsBridge, AssetsBridgeAdminOrOwner {
    ...
    function transferToMagnet(uint256 amount, string calldata ss58Address) external {
        _burn(msg.sender, amount);

        bytes4 selector = bytes4(keccak256("transferToMagnet(address,uint256,string)"));
        bytes memory data = abi.encodeWithSelector(selector, address(this), amount, ss58Address);
